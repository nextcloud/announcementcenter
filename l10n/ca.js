OC.L10N.register(
    "announcementcenter",
    {
    "New comment …" : "Nou comentari...",
    "Delete comment" : "Esborra comentari",
    "Post" : "Publica",
    "Cancel" : "Cancel·la",
    "Edit comment" : "Edita el comentari",
    "[Deleted user]" : "[Usuari eliminat]",
    "Comments" : "Comentaris",
    "No comments yet, start the conversation!" : "No s\\'han enregistrat comentaris encara, comença la conversació!",
    "More comments …" : "Més comentaris...",
    "Save" : "Guardar",
    "Allowed characters {count} of {max}" : "{count} caràcters permesos de {max}",
    "Error occurred while retrieving comment with ID {id}" : "Hi ha hagut un error en llegir el comentari amb id {id}",
    "Error occurred while updating comment with id {id}" : "Hi ha hagut un error en actualitzar el comentari amb id {id}",
    "Error occurred while posting comment" : "Hi ha hagut un error en publicar el comentari",
    "_%n comment_::_%n comments_" : ["%n comentari","%n comentaris"],
    "Delete" : "Esborrar",
    "Remove notifications" : "Esborra les notificacions",
    "Announcing…" : "Publicant...",
    "Announced!" : "Publicat!",
    "Visible for everyone" : "Visible per a tothom",
    ", " : ",",
    "Visible for groups: {groups}" : "Visible per a grups: {groups}",
    "Groups" : "Grups",
    "You announced {announcement}" : "Has anunciat {announcement}",
    "{actor} announced {announcement}" : "{actor} ha anunciat {announcement}",
    "You posted an announcement" : "Has publicat un anunci",
    "{actor} posted an announcement" : "{actor} ha publicat un anunci",
    "Announcement does not exist anymore" : "L'anunci ja no existeix",
    "An <strong>announcement</strong> is posted by an administrator" : "Un administrador ha publicat un <strong>anunci</strong>",
    "Announcements" : "Anuncis",
    "The subject is too long or empty" : "L'assumpte és massa llarg o buit",
    "{user} announced “{announcement}”" : "{user} anuncia {announcement}",
    "%1$s announced “%2$s”" : "%1$s ha anunciat “%2$s”",
    "Announcement center" : "Centre d'anuncis",
    "This app allows admins to post announcements." : "Aquesta app permet als administradors fer anuncis",
    "This app allows admins to post announcements. Also supports the following features:\n\n⚡ Activities (Requires the activity app to be enabled)\n\n📑 Comments\n\n👪 Group permissions\n\n🔔 Notifications (Requires the notifications app to be enabled)" : "Aquesta app permet als administradors fer anuncis. També permet fer les següents funcions:\n\nActivitats (cal tenir l'app d'activitats activada)\n\nComentaris\n\nPermisos de grup\n\nNotificacions (cal tenir l'app de notificacions activada)",
    "These groups will be able to post announcements." : "Aquests grups podran publicar anuncis",
    "Create activities by default" : "Crea activitats per defecte",
    "Create notifications by default" : "Crea notificacions per defecte",
    "Allow comments by default" : "Permet comentaris per defecte",
    "No Announcements" : "Sense Anuncis",
    "There are currently no announcements…" : "Actualment no hi ha anuncis...",
    "Add announcement" : "Afegeix anunci",
    "Subject…" : "Assumpte...",
    "Your announcement…" : "El teu anunci...",
    "Groups…" : "Grups...",
    "These groups will be able to see the announcement. If no group is selected, all users can see it." : "Aquests grups podran veure l'anunci. Si no has escollit cap grup, tots els usuaris el podran veure.",
    "Announce" : "Anunci",
    "Advanced options" : "Opcions avançades",
    "Create activities" : "Crear activitats",
    "Create notifications" : "Crear notificacions",
    "Allow comments" : "Permet comentaris",
    "Error occurred while retrieving comment with id {id}" : "Hi ha hagut un error en extraure el comentari amb id {id}"
},
"nplurals=2; plural=(n != 1);");
