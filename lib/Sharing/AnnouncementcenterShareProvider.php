...
class AnnouncementcenterShareProvider implements IShareProvider
{
	...
	{
		$announcementId = $share->getSharedWith();
		$existingShares = $this->getSharesByPath($share->getNode());
		foreach ($existingShares as $existingShare) {
			if ($existingShare->getSharedWith() === $share->getSharedWith()) {
				throw new GenericShareException('Already shared', $this->l->t('Path is already shared with this announcement'), 403);
			}
		}
	...
	public function getSharedWith($userId, $shareType, $node, $limit, $offset): array
	{
		...
		$qb->select(
			...
			->leftJoin('s', 'filecache', 'f', $qb->expr()->eq('s.file_source', 'f.fileid'))
			->leftJoin('f', 'storages', 'st', $qb->expr()->eq('f.storage', 'st.numeric_id'))
			->leftJoin('s', 'announcements', 'a', $qb->expr()->eq($qb->expr()->castColumn('a.announcement_id', IQueryBuilder::PARAM_STR), 's.share_with'));
	 ...
	 * @return IShare[]
	 */
	public function getSharedWithByType(int $announcementId, int $shareType, $limit, $offset): array
	{
		/** @var IShare[] $shares */
		$shares = [];
	...
	public function getAccessList($nodes, $currentAccess)
	{
		...
		while ($row = $cursor->fetch()) {
			...
			if ($type === IShare::TYPE_DECK) {
				$announcementId = $row['share_with'];
				// $boardId = $this->announcementMapper->findBoardId($announcementId);
				// if ($boardId === null) {
				// 	continue;
				// }
	...
	{
		// Announcementcenter shares when the user has a share exception
