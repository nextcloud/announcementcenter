<template>
	<div id="text-wrapper" class="richEditor">
		...
		<div id="text" class="editor">
			<PageInfoBar :current-page="currentAnnouncement" />
...
export default {
	...
	computed: {
		...mapGetters([
			'collectiveParam',
			'currentCollective',
			'currentAnnouncementDirectory',
			'currentAnnouncementFilePath',
			'isIndexPage',
			'isPublic',
			'loading',
			'pageById',
		...
		attachmentResolver() {
			return new AttachmentResolver({
				fileId: this.currentAnnouncement.id,
				currentDirectory: '/' + this.currentAnnouncementDirectory,
				user: getCurrentUser(),
	...
	methods: {
		...
		handleRelativeFileLink({ href }) {
			if (!href.match(/^[a-zA-Z]*:/)) {
				...
				const path = resolvePath(`/${this.currentAnnouncementFilePath}`, relPath)
